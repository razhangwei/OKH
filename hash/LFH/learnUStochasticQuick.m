function [Ubest, MAPIter, LIter] = learnUStochasticQuick (U0, calcS, beta, batchSize, maxIter, convergeThresh, windowSize, calcMAP)
  ...
  vS = calcS(Si, Sj);
  vT = calcVT(U, Si, Sj);
  L = sum(vS .* vT - logExpTrick(vT)) - sum(sum(U .^ 2, 2)) / (2 * beta);
